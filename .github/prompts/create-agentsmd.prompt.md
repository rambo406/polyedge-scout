...
AGENTS.md files follow a hierarchical, recursive structure similar to how `.gitignore` or `README.md` files work in subdirectories.

### Core Hierarchy Rules

1. **Root AGENTS.md = Generic Guidelines**
   - The root `AGENTS.md` contains guidelines applicable to the **entire project**
   - These are cross-cutting concerns: architecture principles, communication style, general coding philosophy
   - Example: "Use dependency injection", "Follow SOLID principles", "Prefer composition over inheritance"

2. **Subdirectory AGENTS.md = Domain-Specific Guidelines**
   - Subdirectory files (e.g., `frontend/AGENTS.md`, `backend/AGENTS.md`) contain guidelines **specific to that domain**
   - These focus on technology-specific patterns, conventions, and workflows
   - Example: `frontend/AGENTS.md` covers Angular patterns, TypeScript conventions, component architecture

3. **Recursive Scope**
   - Each `AGENTS.md` applies to its directory **and all subdirectories**
   - A file at `src/api/AGENTS.md` governs `src/api/`, `src/api/controllers/`, `src/api/services/`, etc.
   - Agents should look for the closest `AGENTS.md` in the directory tree

4. **No Duplication - Single Source of Truth**
   - Each guideline appears in **exactly one place**
   - Never copy guidelines from root to subdirectory files
   - Subdirectory files reference the root for general principles: *"See root AGENTS.md for general architecture principles"*

5. **Specificity Wins (Optional Override)**
   - When a subdirectory guideline conflicts with a root guideline, the **more specific** one takes precedence
   - This allows domain-specific exceptions to general rules
   - Document overrides explicitly: *"Unlike the general preference for X, this domain uses Y because..."*

### Directory Structure Example

```
project/
├── AGENTS.md                    # Generic: architecture, philosophy, cross-cutting concerns
├── frontend/
│   └── AGENTS.md                # Domain: Angular, TypeScript, component patterns
├── backend/
│   ├── AGENTS.md                # Domain: .NET, C#, API patterns
│   └── src/
│       └── Domain/
│           └── AGENTS.md        # Sub-domain: DDD patterns, aggregate rules (optional)
└── e2e/
    └── AGENTS.md                # Domain: Playwright, testing patterns
```

### Content Separation Guidelines

| Root AGENTS.md | Subdirectory AGENTS.md |
|----------------|------------------------|
| Project vision and philosophy | Technology-specific conventions |
| General architecture principles | Framework-specific patterns |
| Cross-cutting coding standards | Domain-specific workflows |
| Communication style | Tool-specific commands |
| Git and PR conventions | Local testing procedures |
| Overall project structure | Package/dependency management |

### Referencing Parent AGENTS.md

Subdirectory files should reference the root for inherited guidelines:

```markdown
# frontend/AGENTS.md

> **Note**: This file contains frontend-specific guidelines. For general project principles, 
...
### When to Create a Subdirectory AGENTS.md

Create a new nested `AGENTS.md` when:
- A directory uses a **different technology stack** (frontend vs backend)
- The domain has **unique conventions** that don't apply elsewhere
- There are **specific commands or workflows** for that area
- The subdirectory is large enough to warrant its own documentation

**Don't create** a nested file just to have one - only when there's domain-specific value to add.
